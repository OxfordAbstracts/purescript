...
module Language.PureScript.Lsp.Handlers.References where
...
-- referenceHandler :: Server.Handlers HandlerM
-- referenceHandler = Server.requestHandler Message.SMethod_TextDocumentReferences $ \req res -> do

--   let Types.ReferenceParams docIdent pos _prog _prog' ctx = req ^. LSP.params
--       filePathMb = Types.uriToFilePath $ docIdent ^. LSP.uri
--       includeDeclaration = ctx ^. LSP.includeDeclaration

--   res $ Right $ Types.InL $  [ Types.Location _ _ ]
