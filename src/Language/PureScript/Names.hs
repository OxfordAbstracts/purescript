...
instance ToField (OpName a) where
  toField  (OpName a) = toField a

instance FromField (OpName a) where
  fromField a = OpName <$> fromField a
...
instance ToField (ProperName a) where
  toField  (ProperName a) = toField a

instance FromField (ProperName a) where
  fromField a = do 
    n <- fromField a
    if isUpper $ T.head n then
      pure $ ProperName n
    else
      fail "ProperName must be capitalized"
-- |
