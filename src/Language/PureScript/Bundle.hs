...
data ErrorMessage
  ...
  deriving (Show, Generic, S.Serialise, A.FromJSON, A.ToJSON, NFData)
...
instance A.FromJSON ModuleIdentifier where
  parseJSON = A.withObject "ModuleIdentifier" $ \o -> do
    name <- o A..: "name"
    mt :: String <- o A..: "type"
    case mt of
      "Regular" -> pure $ ModuleIdentifier name Regular
      "Foreign" -> pure $ ModuleIdentifier name Foreign
      _ -> fail "Invalid module type"
  ...
  where
    ...
    go (JSModuleStatementListItem jsStatement)
      | Just props <- matchExportsAssignment jsStatement =
          do
            cjsExports <- traverse toIdent (trailingCommaList props)
