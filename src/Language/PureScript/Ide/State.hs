...
resolveLocationsForModule (defs, types) =
  ...
  where
    ...
    convertDeclaration (IdeDeclarationAnn ann d) =
      convertDeclaration'
        annotateFunction
        annotateValue
      ...
      where
        annotateFunction x =
          IdeDeclarationAnn
            ( ann
                { _annLocation = Map.lookup (IdeNamespaced IdeNSValue (P.runIdent x)) defs,
...
convertDeclaration' ::
  (P.Ident -> IdeDeclaration -> t) ->
  (Text -> IdeDeclaration -> t) ->
  (Text -> IdeDeclaration -> t) ->
  (Text -> IdeDeclaration -> t) ->
  (Text -> IdeDeclaration -> t) ->
  (Text -> IdeDeclaration -> t) ->
...
resolveDocumentationForModule (P.Module _ moduleComments moduleName sdecls _) =
  ...
  where
    ...
    comments =
      Map.insert (P.ModName moduleName) moduleComments $
        Map.fromListWith (flip (<>)) $
          concatMap extractDeclComments sdecls
  ...
  where
    ...
    go (ideInstance, classModule, className) acc' =
      let matchTC =
       ...
       in acc' & ix classModule %~ updateDeclaration
  ...
  where
    ...
    dtors =
      decls
        ...
        & foldr
          ( \(IdeDataConstructor name typeName type') ->
              Map.insertWith (<>) typeName [(name, type')]
          )
          Map.empty
