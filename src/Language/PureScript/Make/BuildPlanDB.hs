module Language.PureScript.Make.BuildPlanDB
...
buildJobSuccess :: BuildJobResult -> Maybe MultipleErrors
buildJobSuccess (BuildJobSucceeded warnings) = Just warnings
...
collectResults buildPlan = do
  let prebuiltResults = M.map (const $ BuildJobSucceeded (MultipleErrors [])) (bpPrebuilt buildPlan)
