...
import Language.PureScript.AST (Module, SourcePos (..))
...
import Language.PureScript.Make.Monad (Make, copyFile, getTimestamp, getTimestampMaybe, hashFile, makeIO, readExternsFile, readJSONFile, readTextFile, writeJSONFile, writeTextFile)
...
data MakeActions m = MakeActions
  { -- | Get the timestamps and content hashes for the input files for a module.
    ...
    codegen :: Environment -> CheckState -> Module -> CF.Module CF.Ann -> Docs.Module -> SupplyT m (),
...
writePackageJson' outputDir =
  writeJSONFile (outputDir </> "package.json") $
    object
      [ "type" .= String "module"
      ]
...
buildMakeActions outputDir filePathMap foreigns usePrefix =
  ...
  where
    ...
    codegen :: Environment -> CheckState -> Module -> CF.Module CF.Ann -> Docs.Module -> SupplyT Make ()
    codegen _prevEnv _endEnv _m m docs = do
      ...
      when (S.member JS codegenTargets) $ do
        foreignInclude <- case mn `M.lookup` foreigns of
          ...
          Nothing
            | requiresForeign m -> throwError . errorMessage' (CF.moduleSourceSpan m) $ MissingFFIModule mn
            | otherwise -> return Nothing
        rawJs <- J.moduleToJs m foreignInclude
        putErrText "codegen 3"
...
checkForeignDecls m path = do
  ...
  let parseResult :: Either MultipleErrors JS.JSAST
