...
run :: IO Int
run = flip E.catches execptionHandlers $ do
  ...
  where
    ...
    ioExcept (e :: E.IOException) = IO.print e >> return 1
    someExcept (e :: E.SomeException) = IO.print e >> return 1
...
lspHandlers logger rin = mapHandlers goReq goNot (handlers logger)
  where
    goReq :: forall (a :: LSP.Method LSP.ClientToServer LSP.Request). LSP.Server.Handler (LspM Config) a -> LSP.Server.Handler (LspM Config) a
    goReq f msg k = do
      env <- getLspEnv
      liftIO $ atomically $ writeTChan rin $ ReactorAction (runLspT env $ f msg k)

    goNot :: forall (a :: LSP.Method LSP.ClientToServer LSP.Notification). LSP.Server.Handler (LspM Config) a -> LSP.Server.Handler (LspM Config) a
    goNot f msg = do
      env <- getLspEnv
      liftIO $ atomically $ writeTChan rin $ ReactorAction (runLspT env $ f msg)

-- | Where the actual logic resides for handling requests and notifications.
handlers :: (m ~ LspM Config) => L.LogAction m (WithSeverity T.Text) -> Handlers m
handlers logger =
  mconcat
    [ notificationHandler LSP.SMethod_Initialized $ \_msg -> do
        ...
        void $ sendRequest LSP.SMethod_WindowShowMessageRequest params $ \case
            ...
            makeCommand d
              | (LSP.Range s _) <- d ^. LSP.range,
                (Just "lsp-hello") <- d ^. LSP.source =
                  let title = fromMaybe "" $ Just "Apply LSP hello command:" <> head (T.lines $ d ^. LSP.message)
