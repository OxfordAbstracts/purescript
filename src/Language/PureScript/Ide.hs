...
import Language.PureScript.Ide.State (getAllModules, getFocusedModules, getLoadedModulenames, insertExterns, insertModule, populateVolatileState, populateVolatileStateSync, resetIdeState, setFocusedModules)
...
loadModules moduleNames = do
  ...
  oDir <- outputDirectory
  let -- But we only load the externs files that are in the focusedModules.
      efModules =
        if Set.null focusedModules
          then moduleNames
          else Set.toList $ Set.fromList moduleNames `Set.intersection` focusedModules
      efPaths =
